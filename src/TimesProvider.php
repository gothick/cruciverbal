...
class TimesProvider implements iCrosswordProvider {
    ...
    public function getPdfStreams() {
        ...
        $response = $this->client->request(
            'POST',
            'https://login.thetimes.co.uk/?gotoUrl=https%3A%2F%2Fwww.thetimes.co.uk%2Fpuzzleclub%2Fcrosswordclub%2F',
            [
                'form_params' => [
     ...
     * of Guzzle Streams.
     * 
     * @param \GuzzleHttp\Psr7\Stream $homepage HTML of the Crossword Club Homepage
    ...
    function findStreams($homepage) {
        ...
        if ($day_of_week == 0) {
            ...
            $print_url = $qp
            ->find('h3:contains("Sunday Times Cryptic"):first')
            ->closest('.PuzzleItem')
            ->find('.PuzzleItem--print-link a')
            ->attr('href');
            
            $streams[] = (new \GuzzleHttp\Psr7\Request('GET', $print_url))->getBody();
            
            $print_url = $qp
            ->find('h3:contains("Mephisto"):first')
            ->closest('.PuzzleItem')
            ->find('.PuzzleItem--print-link a')
            ->attr('href');
            ...
            $print_url = $qp
            ->find('h3:contains("Times Cryptic No"):first')
            ->closest('.PuzzleItem')
            ->find('.PuzzleItem--print-link a')
            ->attr('href');
            $streams[] = (new \GuzzleHttp\Psr7\Request('GET', $print_url))->getBody();
